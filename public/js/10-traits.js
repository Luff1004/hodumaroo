...
function showUpgOv(){
  ...
  const ec=Math.floor((100+(shopLv['sh_coin']||0)*20)*coinMult*(partyBonus||1)*(window._petCoinMult||1)*(window._relicCoinMult||1)*(window._merchCoinMult||1));
  const ee=Math.floor((100+(shopLv['sh_energy']||0)*30)*enMult*(partyBonus||1)*(window._petEnergyMult||1)*(window._merchEnergyMult||1));
